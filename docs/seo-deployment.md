...
7. **Кэш.** `pnpm check:cache:prod` — директивы кэша видны в `cdn-cache-control`,
   страница отдаётся эдж-кэшем (`x-vercel-cache: HIT` или `STALE`), картинки
   `/_vercel/image` кэшируются надолго, корень закрыт от общего кэша.
   Устройство и почему директивам нужно зеркало — `docs/dev-guide.md` §8.

   Отдельно — заменённые картинки. Если в релизе файл перезаписан под тем же
   именем, после деплоя стоит позвать `pnpm media:purge /media/…jpg`: на Vercel
   версия в адресе варианта до оптимизатора не доходит, и до истечения срока
   кэша может отдаваться прежняя копия (разбор — `docs/changes-log.md` §77).
   В CI это делает `scripts/postdeploy.mjs` сразу после подтверждённого деплоя,
   если в секретах репозитория есть `VERCEL_TOKEN`; без секрета шаг
   пропускается и печатает одну строку в лог.
...
Подключается через публичную runtime-переменную — ID счётчика **не хардкодится** в коде:

| Переменная | Назначение | Пример |
| --- | --- | --- |
| `NUXT_PUBLIC_YM_ID` | ID счётчика Яндекс.Метрики | `98765432` |

Механика (см. `app/plugins/yandex-metrika.client.ts`):

- Скрипт Метрики грузится **только на клиенте** и **только если переменная задана**.
- Без `NUXT_PUBLIC_YM_ID` сайт не грузит скрипт Метрики и не шлёт никаких запросов
  к `mc.yandex.ru` — плагин выходит без действий.
- Стандартные опции: `clickmap`, `trackLinks`, `accurateTrackBounce`.

Настройка в Vercel:

1. Создать счётчик в [metrika.yandex.ru](https://metrika.yandex.ru) (тип «Сайт», адрес `https://gluke.ru`).
2. Project Settings → Environment Variables → добавить `NUXT_PUBLIC_YM_ID` = номер счётчика
   (для Production; для Preview можно не задавать или указать тестовый счётчик).
3. Пересобрать проект (`git push` — CI сам соберёт и задеплоит).
4. Проверка: в Метрике открыть счётчик → «Проверить счётчик»; статус покажет, что код на сайте.

Как проверить, что код реально на сайте: в браузере открыть исходник страницы и найти
`mc.yandex.ru/metrika/tag.js`. Если переменная не задана — этого фрагмента в HTML нет.
