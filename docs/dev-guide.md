...
- Кейс с `configurator` в шапке показывает обложку, а интерактивная лаборатория
  (`app/components/projects/ProjectCarConfigurator.vue`) стоит отдельным блоком
  ниже: в шапке витрина, все настройки — в лаборатории.
...
- `audio` — музыка зала, один трек на кейс (`app/utils/garageAudio.ts`). Трек не
