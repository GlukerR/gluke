...
## Хранилище деплоев (Deployment Storage)

Бесплатный план даёт 10 ГБ «Deployment Storage», и этот счётчик — не вес
проекта на диске. Платформа хранит **вывод сборки каждого сохранённого
деплоя**: статику (наш `public/` — около 300 МБ, из них 206 МБ видео и 49 МБ
png) плюс бандл функции. Функции считаются отдельной строкой (Functions
Storage), но упираются в тот же лимит.

Три множителя: сколько деплоев сохранено, сколько занимает вывод одного
деплоя, сколько живёт сам деплой. По умолчанию на Hobby все категории
(превью, прод, отменённые, упавшие) живут **30 дней**, деплой создаётся на
каждый пуш и сразу в двух проектах — поэтому счётчик и растёт.

Что сделано в проекте:

1. **Сроки хранения** — `pnpm retention --apply` (нужен `VERCEL_TOKEN` из
   Account Settings → Tokens; без него команда печатает те же шаги для
   дашборда: Project → Settings → Security → Deployment Retention Policy).
   Задумано прод 1 неделя, превью/отменённые/упавшие — сутки; в проекте `gluke`
   (gluke.ru) это уже применено и подтверждено чтением ответа API. Команда ходит
   по проектам **своей команды**: второй сайт, `gluke.vercel.app`, живёт на другом
   аккаунте Vercel, поэтому в этот список не попадает и на квоту этого аккаунта
   не влияет — там политику ставит владелец того аккаунта. Тот же токен,
   положенный в секреты GitHub (`Settings → Secrets → Actions → VERCEL_TOKEN`),
   включает авто-пурж картинок после деплоя — без секрета этот шаг просто
   пропускается.
2. **Пропуск сборки** — `ignoreCommand` в `vercel.json` (`scripts/ignore-build.mjs`).
   Пуш, в котором менялись только документация и служебные файлы, деплоя не
   создаёт вовсе. Правило одностороннее: незнакомый путь (например, новая
   папка исходников) сборку включает, потому что пропущенный релиз заметить
   труднее, чем лишнюю сборку.
3. **Трим бандлов** — после каждой прод-сборки `scripts/prune-server-bundle.mjs`
   (вызывается из `scripts/nuxt-run.mjs`) удаляет платформенные бинарники чужих
   систем: `better-sqlite3` везёт prebuilds под восемь платформ, нужен один.
   Обходятся **оба** вида вывода: `.output/server` (сервер Nitro, из него
   поднимается локальное превью) и `.vercel/output/functions/*` (пресет Vercel
   кладёт туда отдельный `node_modules`; функции ссылаются на одну настоящую,
   поэтому трим считает её один раз). Выигрыш на сборку: −14,2 МБ на Windows,
   около −11,7 МБ на linux-x64 (на Vercel остаются только `linux-x64.node` и
   страховочный `linuxmusl-x64.node`).

Как смотреть и что проверить:

- Сроки хранения уходят и приходят разными именами полей: запрос принимает метки
  (`expiration`, `expirationProduction`, …), а `GET /v2/projects/{id}` отвечает
  днями в `expirationDays*`. Читать политику по полям запроса — значит видеть
  «не задан» там, где всё задано. `pnpm retention` читает оба набора, а после
  `--apply` подтверждает результат повторным чтением проекта, а не отправленным
  телом запроса.
- Разбивка: Team → Usage → Deployment Storage → Projects. Там видно, какой
  проект сколько занимает; внутри деплоя — Deployments → Resources (статика и
  размер каждой функции).
- Эффект от сроков появляется не сразу: удаление запускает фоновая задача, она
  разбирает кандидатов в течение 48 часов и пересматривает исключения (последние
  деплои и активные алиасы) до 30 дней. Кэш картинок и содержимое страниц от
  этой чистки не зависят.
- Если счётчик сильно больше суммы по проектам (у Vercel в сентябре 2026 на
  Hobby это массовая жалоба), речь про задержку учёта после удаления деплоев и
  проектов — там помогает ожидание или обращение в поддержку, а не новые правки.

Видео и картинки из `public/` не сжимались осознанно: это 206 МБ mp4 кейса
cinematics, и решение про их пережатие или вынос во внешнее хранилище остаётся
за владельцем проекта, а не за оптимизацией хранилища.
