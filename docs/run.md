...
- Счётчик — это **ГБ-месяцы**: Vercel складывает по дням периода максимум
  хранения за день. Поэтому внутри периода он только растёт (даже без новых
  деплоев) и не уменьшается ни от чистки, ни от смены политики — увидеть эффект
  сроков можно лишь с началом нового периода. Один деплой здесь ≈ 0,33 ГБ
  (статика 296 МБ плюс функции).
