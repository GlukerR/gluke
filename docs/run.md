...
1. **Сроки хранения** — `pnpm retention --apply` (нужен `VERCEL_TOKEN` из
   ...
   Задумано прод 1 неделя, превью/отменённые/упавшие — сутки. Политику надо
   поставить в **обоих** проектах: gluke.ru и gluke.vercel.app собирают один
   репозиторий, то есть место съедают дважды. Тот же токен, положенный в
   секреты GitHub (`Settings → Secrets → Actions → VERCEL_TOKEN`), включает
   авто-пурж картинок после деплоя — без секрета этот шаг просто пропускается.
