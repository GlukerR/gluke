/**
 * Production build в отдельном build-каталоге.
 *
 * `nuxt build` и `nuxt dev` по умолчанию делят корневой `.nuxt`, и работающий
 * dev-сервер переписывает туда свои dev-шаблоны: например, `@nuxt/icon`
 * генерирует `nuxt-icon-server-bundle.mjs` с `createRequire(import.meta.url)`
 * только в dev-режиме (`isBundling = !dev`). Если прод-сборка подхватывает
 * такой шаблон, nitro заменяет `import.meta.url` на плейсхолдер
 * `"file:///_entry.js"` и prerender падает на Windows (createRequire не
 * принимает этот путь). Отдельный buildDir изолирует сборку от dev-сервера:
 * `pnpm build` работает даже при запущенном dev (и после `rm -rf .nuxt`).
 */
...
if (!existsSync('.nuxt/tsconfig.app.json')) {
  console.log('[build] .nuxt is missing — running nuxt prepare first')
