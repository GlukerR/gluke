...
async function mount() {
  ...
  try {
    ...
    prism.value = Pleprism.create(el, {
      ...
      hover: !coarse,
      /* Курсор считываем от самого блока, а не от окна. Виджет нормирует
         положение по своему прямоугольнику: `nx = (clientX - rect.left) /
         rect.width * 2 - 1`. В оригинальном демо элемент занимает весь экран,
         поэтому по окну выходит плавный ход от -1 до 1. Здесь блок 16:9 в
         середине страницы — при отслеживании по окну `nx` мгновенно вылетает
         за границы и упирается в ограничитель ±1.6: призма прижата к краю и не
         реагирует. Привязка к блоку даёт полный ход по его ширине. */
      hoverTarget: 'self',
    }) as PrismInstance
