...
/* Наполнение знака энергией. Фирменный жёлтый один и тот же в обеих темах —
   это цвет бренда, его не подкручивают. Тема меняет то, что вокруг: на
   тёмном фоне погасший знак должен быть светлее фона, на светлом — темнее,
   иначе исходное состояние сливается и «потенциал перед активацией» не
   читается. Ореол на светлом фоне режем: аддитивное свечение по белому
   выцветает в молоко. */
export const ENERGY_FILL_THEME_LOOK: Record<'dark' | 'light', WidgetThemeLook> = {
  dark: {
    lightness: 1,
    dormant: 0.3,
    halo: 1,
  ...
  light: {
    lightness: 0.86,
    dormant: 0.62,
    /* Плотный ореол в светлой теме гасить почти не нужно: он узкий и жёлтый,
       по светлому фону читается как свет, а не как грязь. */
    halo: 0.9,
    rim: 0.5,
  },
}
