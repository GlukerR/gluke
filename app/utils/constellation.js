...
const DEFAULTS = {
  ...
  countMult: 1.35,   // множитель плотности: 1 — стандарт для всех вариантов
  ...
  // --- связи ---
  linkDist: 0.09,   // порог связи, доля меньшей стороны кадра
...
class Constellation {
  ...
  _step(now) {
    ...
    const linkR = opts.linkDist * minSide
  ...
  _draw(now) {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      const gx = p.x < edgeM ? clamp(1 - p.x / edgeM, 0, 1)
        : p.x > w - edgeM ? clamp(1 - (w - p.x) / edgeM, 0, 1) : 0
      const gy = p.y < edgeM ? clamp(1 - p.y / edgeM, 0, 1)
